...
### v1.3.19 -> v1.3.20: Tauri 빌드 시 PWA 완전 배제

*   **문제**: PWA(VitePWA) 플러그인이 서비스 워커와 매니페스트를 주입하는데, 이것이 `file://` 프로토콜 기반의 Tauri 환경에서 캐싱 문제나 로딩 충돌을 일으킬 가능성이 제기됨.
*   **해결 시도**:
    *   **Tauri 전용 빌드 스크립트**: `package.json`에 `"build:tauri": "tsc && VITE_IS_TAURI=true vite build"` 추가.
    *   **PWA 플러그인 조건부 비활성화**: `vite.config.ts`에서 `VITE_IS_TAURI`가 참일 경우 `VitePWA` 플러그인을 아예 로드하지 않음.
    *   **서비스 워커 등록 방지**: `src/main.tsx` 및 `src/platforms/web/index.tsx`에서 `VITE_IS_TAURI` 환경 변수를 체크하여 서비스 워커 등록 코드를 건너뜀.
    *   **빌드 파이프라인 연결**: `tauri.conf.json`의 `beforeBuildCommand`를 `npm run build:tauri`로 변경.
*   **기대 효과**: 순수한 React 빌드 결과물만 Tauri에 포함되어, PWA 관련 충돌 요소를 원천 제거.

### v1.3.20 -> v1.3.21: Windows(CI) 빌드 환경 호환성 수정

*   **문제**: GitHub Actions(Windows) 또는 로컬 Windows 환경에서 `npm run build:tauri` 실행 시 `'VITE_IS_TAURI' is not recognized` 에러 발생.
    *   원인: `VITE_IS_TAURI=true vite build` 문법은 Unix 쉘(Linux/Mac) 전용이며, Windows cmd에서는 동작하지 않음.
*   **해결 시도**:
    *   **`cross-env` 설치**: `npm install -D cross-env`.
    *   **스크립트 수정**: `package.json`의 `build:tauri`, `build:webapp`, `build:extension` 스크립트를 `cross-env VAR=val command` 형태로 변경.
*   **결과**: Windows를 포함한 모든 환경에서 환경 변수 주입이 정상 작동하여 빌드가 성공할 것으로 예상됨.