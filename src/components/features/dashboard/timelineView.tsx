  ...
  const { timelineItems, totalReminders, completedReminders } = useMemo(() => {
    ...
    notes.forEach((note) => {
      ...
      items.push({
        ...
        date: new Date(note.created_at), // 노트는 마지막 활동(수정) 시간을 기준
        tags: note.tags || [],
        noteId: note.id,
        originalData: note,
      });
      ...
      (note.reminders || []).forEach((reminder: any) => {
        // 'any' 타입으로 캐스팅하여 created_at 접근
        items.push({
          id: `reminder-${reminder.id}`,
          type: 'reminder',
          title: reminder.text || reminder.reminder_text, // 데이터 형식 호환
          date: new Date(reminder.created_at), // ✅ 변경된 부분: 리마인더 생성 시간을 기준
