...
import { convertObsidianLinksToMarkdown } from "@/utils/obsidianLinks";
...
export const MarkdownPreview: React.FC<MarkdownPreviewProps> = React.memo(
	({ content, onWikiLinkClick }) => {
		const processedContent = React.useMemo(() => {
			const withWikiLinks = convertObsidianLinksToMarkdown(content || "");
			return preserveNewlines(withWikiLinks);
		}, [content]);
		...
		return (
			<div className="prose prose-sm max-w-none dark:prose-invert">
				<ReactMarkdown
					...
					components={{
						h1: ({ children, ...props }) => (
							<h1 id={getHeaderId(children)} className="text-4xl font-extrabold mb-6" {...props}>
								{children}
							</h1>
						),
						h2: ({ children, ...props }) => (
							<h2 id={getHeaderId(children)} className="text-2xl font-bold mb-4" {...props}>
								{children}
							</h2>
						),
						h3: ({ children, ...props }) => (
							<h3 id={getHeaderId(children)} className="text-lg font-semibold mb-2 mt-4" {...props}>
								{children}
							</h3>
						),
						p: ({ children, ...props }) => (
						...
						a: ({ children, href, onClick, ...props }) => {
							const isWikiLink =
								typeof href === "string" && href.startsWith("#notia-wiki:");
							...
							const handleClick = (
								...
								if (isWikiLink && typeof href === "string") {
									...
									const encodedTitle = href.slice("#notia-wiki:".length);
                        ...
                        input: (_props) => {
							if (_props.type === "checkbox") {
								const checkIcon = encodeURIComponent(
									'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>',
								);
								return (
									<input
										type="checkbox"
