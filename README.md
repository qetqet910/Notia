...
1. 로그인을 한 상태 dashboard -> login 뒤로가기 버튼으로 갈 경우 redirect X O
2. 그 상태로 다시 dashboard로 갈 경우 무한로딩 발생 (세션 있음, 사용자 데이터 로드 시작) O
...
- 현재 쓸데없는 기능 남발, 코드가 더러움, 목적성이 명확하지 않음

기존 제품의 불편 - todoist chrome extension이 너무 느림, 너무 쉽게 꺼짐, UX 꽤나 불편함
ㄴ개선 방안 - chrome 확장 로딩의 해결, 웹앱으로 사용성 확대, UX 개선

제작 이유 - 개인적인 리마인더가 필요, 너무 잘 까먹음 + 사지방(군대)의 특성상 접근성이 좋아야함 + 가볍고(노션 진입시 최소 3분 소요), 단순하고, 빨라야함
ㄴ개선 방안

- 로그인 - Personality는 보장 가능 But. 접근성이 그닥..
- 개인 URL - 나만 쓸거면 로그인 필요없긴 함, 접근성이 좋지만 But, 프라이버시 보장 장담 못혀
- Claude쨩과 한차례 토론 후 방법을 정함

16자리의 키를 생성(게시글에 비번거는 비회원 느낌) 각 기기에서 로그인 시 키를 입력하여 리마인더 사용 + Google, Github Auth 동시 지원 = 개인성, 접근성, 서비스를 사용하기까지 걸리는 시간 감소 - 요 방법 채택 = 플젝 갈아엎기.. .. ...... . ... . .. TSLL4.3층 ... ... 돈 벌어야지..

근데... 심심해서 만드는 막고라 서비스를 누가 써줄까......??
별로 좋은 것도 없는 것 같은데..
군인들이 써주려나..? 싸지방... 느리니까..... 엣큥★

Todoist - 메인 벤치마킹

- 오늘, 내일, 노트, 그룹 섹션
  - 공통 - 작업 추가, 웹사이트로 작업 추가, 섹션 추가
  * 오늘 / 오늘의 할 일 작성
  * 내일 / 내일 할 일 작성 + 캘린더 형식
  * 노트 / 중장기, 단기적 목표 작성
  * 그룹 / 다른 사람들과 계획, 리마인더 작성
...
- 메모시 태그도 리랜더링이 안 되는 문제 발생 / 위와 같이 해결이용
